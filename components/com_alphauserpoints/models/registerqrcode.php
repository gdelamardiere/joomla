...
class alphauserpointsModelRegisterqrcode extends Jmodel {
	...
	{	
		...
		$login  		= JRequest::getVar('login', '', 'post', 'username');
		$coupon			= JRequest::getVar('couponCode', '', 'post', 'string');
		$trackID		= JRequest::getVar('trackID', '', 'post', 'string');
			...
			{			
				...
				if ( !$result[0]->public )
				{
					...
					else 
					{
						...
						$app->redirect('index.php?option=com_alphauserpoints&view=registerqrcode&QRcode='.$coupon.'&trackID='.$trackID, $msg);	
					}
				} 
