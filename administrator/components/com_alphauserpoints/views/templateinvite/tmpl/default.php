	...
	<table class="adminlist" cellpadding="1">
		<thead>
			<tr>
				...
				<th width="3%" class="title">
					<input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count($this->templateinvite); ?>);" />
				</th>
		...
		<?php		
			...
			for ($i=0, $n=count( $this->templateinvite ); $i < $n; $i++)
			{
				...
				$link 	= 'index.php?option=com_alphauserpoints&amp;task=edittemplateinvite&amp;cid[]='. $row->id. '';
				//$img 	= $row->published ? 'publish_x.png' : 'tick.png';
				$published 	= JHTML::_('grid.published', $row, $i );				
				$format = $row->emailformat ? JText::_( 'AUP_HTML' ) : JText::_( 'AUP_PLAIN-TEXT' );
				$bcc2admin = $row->bcc2admin ? JText::_( 'AUP_YES' ) : JText::_( 'AUP_NO' );
			?>
