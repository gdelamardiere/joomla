...
class alphauserpointsModelCouponcodes extends Jmodel {
	...
	function _save_coupon() {
		...
		if ($post['printable']==1) {
			...
			$urlQR = JURI::root().'index.php?option=com_alphauserpoints&view=registerqrcode&QRcode='.$post['couponcode'];
			createURLQRcodePNG( $urlQR, 50, $pathQR.DS.'50'.DS.$post['couponcode'].'.png' );
			createURLQRcodePNG( $urlQR, 250, $pathQR.DS.'250'.DS.$post['couponcode'].'.png' );
		}
		// msg
