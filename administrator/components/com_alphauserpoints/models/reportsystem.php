...
class alphauserpointsModelReportsystem extends Jmodel {
	...
	function _generate_report()
	{
		...
		$aupversion = _ALPHAUSERPOINTS_NUM_VERSION;
		...
		foreach($tableslist as $table) {
			if ( preg_match('_alpha_', $table) ) {
				...
				if ($db->getErrorMsg()) JError::raiseError( 500, $db->getErrorMsg()) ;
		...
		$aupconfigsettings = JComponentHelper::getParams( 'com_alphauserpoints' );
		...
		$report = '[confidential][b]Joomla! version:[/b] '.$jversion.' [b]Platform:[/b] '.$_SERVER['SERVER_SOFTWARE'].' ('
			...
			.'[quote][b]AlphaUserPoints version:[/b] [u]Installed version:[/u] '.$aupversion
			.chr(13).'[b]AlphaUserPoints detailled configuration:[/b][spoiler]'			
			...
			.'[/spoiler][/quote]'
			.'[quote][b]AlphaUserPoints installed rules:[/b][spoiler]'	
			
			. $this->_checkRules()
			
			.'[/spoiler][/quote]'
			
			.'[quote][b]AlphaUserPoints installed menus:[/b][spoiler]'	
	...
	{	
		...
		$query = "SELECT * FROM #__menu WHERE `link` LIKE '%option=com_alphauserpoints&view=%'";
		$db->setQuery( $query );
