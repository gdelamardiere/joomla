...
class alphauserpointsModelStatistics extends Jmodel {
	...
	function _pointsearned() {
		...
		$query = "SELECT a.referreid, SUM(a.points) AS sumpoints, u.username AS username, u.name AS name"
			   ...
			   . " WHERE aup.referreid=a.referreid AND aup.userid=u.id AND aup.published='1' AND a.approved='1' AND a.status='1' AND a.points>=1"
			   . $where
	...
	function _pointsspent() {
		...
		$query = "SELECT a.referreid, SUM(a.points) AS sumpoints, u.username AS username, u.name AS name"
			   . " FROM #__alpha_userpoints_details AS a, #__alpha_userpoints AS aup, #__users AS u"
			   . " WHERE aup.referreid=a.referreid AND aup.userid=u.id AND aup.published='1' AND a.approved='1' AND a.status='1' AND a.points<0"
			   . $where
			   . " GROUP BY a.referreid "
	...
	function _get_most_country( $limit ) {
		...
		$query = "SELECT country, COUNT(id) AS numusers"
			   . " FROM #__alpha_userpoints"
			   . " WHERE blocked='0' AND userid > 0 AND referreid!='GUEST' AND published='1'"
			   . " GROUP BY country"
	...
	function _get_ratio_gender_members() {
		...
		$query = "SELECT COUNT(id) AS nb, gender FROM #__alpha_userpoints WHERE userid > 0 AND referreid!='GUEST' AND published='1' GROUP BY gender";
		$db->setQuery( $query );
